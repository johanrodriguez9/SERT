...
//	        	redirect('sali', 'refresh');
			site_url(Hasher::make(3));
	        } else {
	            if (!$this->ion_auth->in_group('admin')) { //verifica grupo
//	        	redirect('auth/logout', 'refresh');
	...
	public function editar_grupo(){
		...
		$dato['obj']=$this->db->query("SELECT * FROM dp_auth_groups WHERE id='$id' ")->row();
		$this->load->view("backend/groups/edit",$dato);
	}
	public function guardar_editar_grupo(){
		$id=$this->input->post('id_grupo');
		$group_name1=$this->input->post('group_name1');
		$description1=$this->input->post('description1');

		$this->ion_auth->update_group($id, $group_name1, $description1);
	}
