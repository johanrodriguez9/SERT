...
public function index(){
		...
		{ 
			...
			$this->data['page_content'] = 'backend/Comunidad/portada/index';
			$this->render();
		}
	}
	...
	public function guardar_portada(){
			...
			foreach($_FILES["file"]['tmp_name'] as $key => $tmp_name){
 			...
 			if(!empty($_FILES["file"]['tmp_name'][$key])){ 
				$anterior_img=$this->Modelo_configuracion->tabla_row('tienda_portada','portada_id',$id);
				...
				if ($_FILES["file"]['type'][$key]=="image/jpg" || $_FILES["file"]['type'][$key]=="image/jpeg") {
					...
					if ($_FILES["file"]['type'][$key]=="image/png") {
						...
						$ruta="assets/img_portada/port_".$id.''.$ima;
						$origen=imagecreatefrompng($_FILES["file"]['tmp_name'][$key]);
						$size=1920;
						$ratio = $originalWidth / $originalHeight;
						$targetWidth = $targetHeight = min($size, max($originalWidth, $originalHeight));
						if ($ratio < 1) {
