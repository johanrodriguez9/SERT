    ...
    <div>
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">ADMINISTRACIÓN CATEGORIA MIEMBRO</li>
            </ol>
        </nav>
    </div>
</div>
    ...
    <div class="col-xl-12">
        <div class="card" style=" box-shadow: 0px 5px 10px rgb(52, 73, 94, 0.5);">
            <div class="card-header pb-0">
                <center><h2 class="fer_texto mg-b-0 mt-2" align="center">ADMINISTRACIÓN DE CATEGORIA MIEMBRO TECH LAB BOLIVIA</h2></center>
                <br>
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table text-md-nowrap" id="example1">
                        ...
                        <tbody>
                          <?php $cont=1; foreach ($listar_miembro_categoria as $value): ?>
                              <tr>
                                <th><?= $cont++; ?></th>
                                <th><?= $value->tm_titulo ?></th>
                                <th><?= $value->tm_descripcion ?></th>
                                <th><?php if($value->tm_estado=='1'){ ?>
                                    <div class="form-block">
                                        <input type="checkbox" class="iswitch iswitch-md iswitch-success" checked onclick="cambiar_estado_miembro('<?= $value->tm_id ?>','<?= $value->tm_estado?>')">
                                    </div> Activo
                                <?php }else{ ?>
                                    <div class="form-block">
                                        <input type="checkbox" class="iswitch iswitch-md iswitch-success" onclick="cambiar_estado_miembro('<?= $value->tm_id ?>','<?= $value->tm_estado ?>')">
                                    </div> Inactivo
                                <?php  } ?></th>
                                <th></th>
                             </tr>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content ">
      ...
      <div class="modal-body">
        <div class="row">
             <form  id="guardar_categoria_miembro" class="col-lg-12" method="post" enctype="multipart/form-data">
...
<script type="text/javascript">
  $("#guardar_categoria_miembro").submit(function(event) {

    event.preventDefault();
   $('#loading').modal({backdrop: 'static', keyboard: false});
    
    var formData=new FormData($("#guardar_categoria_miembro")[0]);
    $('#loading').modal({backdrop: 'static', keyboard: false});
    $.ajax({
        url:'<?php echo base_url(Hasher::make(97));?>',
        type:'POST',
        data:formData,
        cache:false,
        processData:false,
        ...
        success:function(){ 
          ...
          setTimeout(function(){
               window.location='<?php echo base_url(Hasher::make(96)); ?>';
          },1000);
        } 
    });       
  });
    ...
    function cambiar_estado_miembro(id,estado){
        $.post('<?php echo base_url(Hasher::make(98)); ?>', {id,estado}, function() {
            swal({
              title: 'NOTA',
