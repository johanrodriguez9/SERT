...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if ($this->identity_check($identity))
		{
			$this->db->where('id_persona',$additional_data['id_persona']);
        	// $this->db->update($this->tables['users'],array('carrera_id'=>$additional_data['carrera_id']) );
	...
	public function registerE($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		...
		elseif ( !$this->config->item('default_group1', 'ion_auth') && empty($groups) )
		{
			$this->set_error('account_creation_missing_default_group');
			return FALSE;
		}
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('default_group1', 'ion_auth')),1)->row();
		if( !isset($query->id) && empty($groups) )
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
	...
	public function registerD($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		...
		elseif ( !$this->config->item('default_group2', 'ion_auth') && empty($groups) )
		{
			$this->set_error('account_creation_missing_default_group');
			return FALSE;
		}
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('default_group2', 'ion_auth')),1)->row();
		if( !isset($query->id) && empty($groups) )
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$this->trigger_events('extra_where');
			$query = $this->db->select($this->identity_column . ', email,username,id_persona, id, password, active, last_login')
                  ->where($this->identity_column, $identity)
                  ->limit(1)
    			  ->order_by('id', 'desc')
                  ->get($this->tables['users']);
	...
	public function users($groups = NULL)
	{
		...
		//verifica usuario; si existe; revisamos carrera para actualizar a este
		$id=$this->session->userdata('user_id');
		$sql="Select id_persona from dp_auth_users where  id='$id' ";
		$query = $this->db->query($sql);
		if( ! $query->num_rows()>0 ){
			/*$sqlr="SELECT v.carrera_id  FROM base_upea.vista_asignacion_control_docente v
				inner join dp_auth_users u on v.id_persona=u.id_persona 
				WHERE u.id = $id AND v.carrera_id = $carrera_id AND v.estado_gestion ='A' ";
			$queryr = $this->db->query($sqlr);
			if( $queryr->num_rows()>0 ){
				//actulizacion
				$this->db->where('id', $id);
				$this->db->update('dp_auth_users', array('carrera_id'=>$carrera_id));
			}*/
		}
		
		//filtro por carrera rgs
		// $this->db->where_in($this->tables['users'] . '.carrera_id',$carrera_id);
	...
	**/
	...
	public function set_session($user)	{
		...
		$session_data = array(
		    ...
		    'email'                		=> $user->email,
		    // 'id_carrera'              => $user->carrera_id, //everyone likes to overwrite id so we'll use user_id
		    'id_persona'              => $user->id_persona, //everyone likes to overwrite id so we'll use user_id
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		if($group_id==0){
			$obj=$this->db->query("SELECT MAX(id) as id FROM dp_auth_groups ")->row();
			$id_grupo=(($obj->id)+1);
			$array=array('id'=>$id_grupo);
			$this->db->WHERE('id',$group_id);
			$this->db->update('dp_auth_groups',$array);
		}
	...
	**/
	...
	public function slect_persona_ci($ci){
       $sql="SELECT id as persona_id, ci, expedido, nombre, paterno, materno, fecha_nac, genero FROM base_upea.vista_persona  WHERE ci = $ci";
        $query = $this->db->query($sql);
		if( $query->num_rows()>0 )
			return $query->row();
		else        
	...
	public function usuario($id){
		$sql="SELECT dp_auth_users.id, dp_auth_users.id_persona
		FROM
		dp_auth_users
		where dp_auth_users.id = $id";
		$query = $this->db->query($sql);
		if( $query->num_rows()>0 )
			return $query->row();
		else        
	...
	/*public function fecha($ids){
		$sql="SELECT
		base_upea.persona.id,
		DATE_FORMAT(base_upea.persona.fecha_nac, '%d-%m-%Y') fechas
		FROM
		base_upea.vista_persona
		WHERE 
		base_upea.persona.id = $ids";
		$query = $this->db->query($sql);
		if( $query->num_rows()>0 )
			return $query->row();
		else        
	...
	public function guardar_nuevo_usuario_pass($id,$name,$password){
		$pas=$this->hash_password($password);
		$obj=array('username'=>$name,'password'=>$pas);
		$this->db->WHERE('id',$id);
		$this->db->update('dp_auth_users',$obj);
	}
	public function generar_usuario(){ 
		$gestion=(date('Y')-1);
		$persona=$this->db->query("SELECT  persona.ci,persona.id,persona.nombre,persona.paterno,persona.materno,persona.fecha_nac,segui.id_persona
		FROM sysdb.dp_auth_users as segui 
		RIGHT JOIN base_upea.vista_persona as persona  ON segui.id_persona=persona.id
		WHERE  YEAR(persona.fecha_registro_persona)<='$gestion' AND segui.id_persona IS NUll")->result();
		$contador=0;
		foreach ($persona as $value) {
			$carrera=$this->db->query("SELECT base_upea.carrera,base_upea.carrera_id FROM base_upea.vista_mae_matriculados as base_upea WHERE base_upea.id_estudiante='$value->id' GROUP BY carrera_id ")->row();
			if ($carrera) {
				$contador=$contador+1;
				$nombre=mb_strtoupper($value->nombre,'utf-8');
				$a=explode(" ", $nombre); 
				$usuario=$a[0].'_'.$value->ci;
				$pass=date("d-m-Y", strtotime($value->fecha_nac));
				$pas=$this->hash_password($pass);
				$var=$this->db->query("SELECT COUNT(id) as cantidad FROM dp_auth_users ")->row();
				$id_u=(($var->cantidad)+1);
				$obj=array(
					'id'=>$id_u,
					'id_persona'=>$value->id,
					...
					'password'=>$pas,
					'active'=>'1',
					'first_name'=>$value->nombre,
					'last_name'=>$value->paterno.' '.$value->materno,
					'company'=>'UPEA',
					'email'=>$usuario
				);
	...
	public function guardar_nuevo_usuario($nombre,$apellido,$id_tipo_usuario,$name,$pass){
		$pas=$this->hash_password($pass);
		$nombre=mb_strtoupper($nombre,'utf-8');
		$valor=$this->db->query("SELECT  MAX(id) as maximo FROM dp_auth_users")->row();
		$id_u=($valor->maximo)+1;
		$obj1=array(
			'id'=>$id_u,
			'id_persona'=>$id_u,
		...
		$this->db->insert('dp_auth_users',$obj1);

		$obj=array(
			'id'=>$id_u,
			'user_id'=>$id_u,
			'group_id'=>$id_tipo_usuario
		);
		$this->db->insert('dp_auth_users_groups',$obj);

	}

	public function guardar_nuevo_rol_usuario($id_grupo,$id){
	...
	public function guardar_editar_usuario($id_usuario1,$nombre,$apellido,$id_tipo_usuario1){
		$obj=array(
			'first_name'=>$nombre,
			'last_name'=>$apellido
		);
		$this->db->WHERE('id',$id_usuario1);
		$this->db->update('dp_auth_users',$obj);

		$obj=array(
			'user_id'=>$id_usuario1,
			'group_id'=>$id_tipo_usuario1
		);
		$this->db->WHERE('id',$id_usuario1);
		$this->db->update('dp_auth_users_groups',$obj);
	}
	public function usuario_listado_todo(){
		return $this->db->query("SELECT * FROM dp_auth_users ")->result();

	}

	public function verificar_grupo_asignado($id_t,$id_u){
		return $this->db->query("SELECT  * FROM dp_auth_users_groups WHERE user_id='$id_u' AND group_id='$id_t' ")->row();
	}
	
	public function tabla_usuario($id_usuario){
		return $this->db->query("SELECT active FROM dp_auth_users WHERE id='$id_usuario' ")->row();
	}
	public function mostrar_grupo($id_usuario){
		return $this->db->query("SELECT
		dp_auth_users_groups.user_id,
		dp_auth_groups.`name`
		FROM dp_auth_users_groups
		INNER JOIN dp_auth_groups ON dp_auth_users_groups.group_id = dp_auth_groups.id
		WHERE dp_auth_users_groups.user_id='$id_usuario' ")->result();
	}
	public function buscar_c($ci){
		$query = $this->db->query("SELECT persona.ci,persona.id,persona.nombre,persona.paterno,persona.materno,persona.fecha_nac
		FROM auth_usuario as persona
		WHERE persona.ci='$ci'");
		
		$query1 = $this->db->query("SELECT miembro_id, miembro_apellidos, miembro_nombres, miembro_correo, miembro_celular, miembro_documento, miembro_pais, miembro_departamento, miembro_ciudad, miembro_carrera, miembro_estado, id_user, tech_tipo_miembro_tm_id 
		FROM tech_miembro 
		WHERE miembro_correo like '$ci'");
	...
	public function eliminar_user_usuario($id_usuario){
		$this->db->query("DELETE FROM dp_auth_users_groups WHERE user_id ='$id_usuario'");
		$this->db->query("DELETE FROM dp_auth_users WHERE id='$id_usuario'");

	}

	public function editt_usuario($id_usuario){
		return $this->db->query("SELECT au.id, au.id_persona, au.carrera_id, au.ip_address, au.username, au.password, au.salt, au.email, au.activation_code, au.forgotten_password_code, au.forgotten_password_time, au.remember_code, au.created_on, au.last_login, au.active, au.first_name, au.last_name, au.company, au.phone, au.imagen 
		FROM dp_auth_users as au WHERE au.id='$id_usuario' ")->row();
	}


	public function reset_usuario($id_usuario){
		return $this->db->query("SELECT *
		FROM dp_auth_users as segui 
		WHERE segui.id='$id_usuario' ")->row();
	}


	public function guardar_reset_usuario($usuario,$pass,$id_usuario){
		$pas=$this->hash_password($pass);
		$obj=array(
			'username'=>$usuario,
			'password'=>$pas,
	...
	public function verificar_usuario_existente($id){
		return $this->db->query("SELECT
		dp_auth_users_groups.user_id,
		dp_auth_users_groups.group_id,
		dp_auth_users.id,
		dp_auth_users.id_persona,
		dp_auth_users.carrera_id,
		dp_auth_users.first_name,
		dp_auth_users.last_name
		FROM dp_auth_users
		INNER JOIN dp_auth_users_groups ON dp_auth_users_groups.user_id = dp_auth_users.id AND dp_auth_users_groups.user_id = dp_auth_users.id
		WHERE dp_auth_users.email like '$id'")->row();
	}

	public function verificar_usuario_existente_id($id){
		return $this->db->query("SELECT
		dp_auth_users_groups.user_id,
		dp_auth_users_groups.group_id,
		dp_auth_users.id,
		...
		FROM dp_auth_users
		INNER JOIN dp_auth_users_groups ON dp_auth_users_groups.user_id = dp_auth_users.id AND dp_auth_users_groups.user_id = dp_auth_users.id
		WHERE dp_auth_users.id = '$id'")->row();
	}

	public function listrar_sedes_grupos(){
		return $this->db->query("SELECT
		base_upea.vista_carrera_sede.id_sede,
		base_upea.vista_carrera_sede.sede,
		base_upea.vista_carrera_sede.area_id,
		base_upea.vista_carrera_sede.estado
		FROM base_upea.vista_carrera_sede
		WHERE base_upea.vista_carrera_sede.id_sede!='0' AND
		base_upea.vista_carrera_sede.id_sede!='14' AND
		base_upea.vista_carrera_sede.id_sede!='15' AND
		base_upea.vista_carrera_sede.id_sede!='23' 
		/*AND base_upea.vista_carrera_sede.estado='A*/
		GROUP BY base_upea.vista_carrera_sede.id_sede ")->result();
	}
